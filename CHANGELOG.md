...
- **App no longer silently fails to start after a reboot.** Root cause: after a restart the custom data directory (Preferences → data dir) can be temporarily unavailable (drive not mounted / OneDrive or network path not ready), which made `DatabaseDriverFactory` return a null database. `ServerManager` dereferenced it during Koin init and crashed *before* any visible window, exiting with no process and no error. `ServerManager.database` is now nullable (`DatabaseRepository?`) and gracefully starts with an empty server list when the DB can't be opened (`loadServersFromDatabase` warns and returns empty).
- **Startup errors are never invisible again.** `main()` now wraps the whole initialization sequence in `fatalStartup()` — any failure writes `<dataDir>/startup-error.log`, prints the stack trace, and shows a `JOptionPane` error dialog with the message and log path before exiting. An uncaught-exception handler covers crashes anywhere else.
- **Database opens are now resilient.** `DatabaseDriverFactory` retries the SQLite open 3× (2 s apart) and enables `PRAGMA journal_mode=WAL` + `PRAGMA busy_timeout=5000`, so a transiently unavailable data dir (or a DB still being closed by the previous process) recovers instead of hard-failing.
- **Single-instance lock is now recoverable and visible.** The lock file stores the owning PID + timestamp; if a leftover lock from a dead process is found, it is cleaned up automatically. If a real second instance is started, a "PortalHost is already running" dialog points at the system tray instead of exiting silently. Duplicate-launch and dead-PID paths both covered by new `SingleInstanceLockTest` cases (full desktop suite 29/29 green).
- **On-disk log.** `setupLogging()` now appends to `<dataDir>/portalhost.log` (logback `FileAppender`) in addition to the in-memory log, so crashes and startup failures leave a trace that survives the process.
